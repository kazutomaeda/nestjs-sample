...
export class OrderController {
  ...
  async findAll(
    ...
    const { items, totalItems } = await this.orderUsecase.findAll(
      ability,
      query,
    );
    return {
  ...
  async create(
    ...
    const order = await this.orderUsecase.create(dto, user.tenantId, user.sub);
    return this.toResponse(order);
  }
  ...
  async update(
    ...
    const ability = this.caslAbilityFactory.createForUser(user);
    const order = await this.orderUsecase.update(
      id,
      dto,
      user.tenantId,
      user.sub,
  ...
  async remove(
    ...
    const ability = this.caslAbilityFactory.createForUser(user);
    const order = await this.orderUsecase.remove(id, user.sub, ability);
    return this.toResponse(order);
  }
